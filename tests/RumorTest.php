...
it('wraps message in a seal and seal in a gift', function () {
    ...
    expect($rumor->id)->toBe(hash('sha256', Nostr::encode([0, $sender_pubkey, $rumor->created_at, 14, [['p', $sender_pubkey]], 'Hello!!'])));
});
...
it('can be restored', function () {
    ...
    $rumor2 = Rumor::__set_state($rumor());

    expect($rumor2)->not()->toHaveProperty('sig');
    expect($rumor2)->toHaveProperty('id');
    expect($rumor2->id)->toBe(hash('sha256', Nostr::encode([0, $sender_pubkey, $rumor2->created_at, 14, [['p', $sender_pubkey]], 'Hello!!'])));
});
